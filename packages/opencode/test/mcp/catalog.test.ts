...
describe("McpCatalog.convertTool (A2 sync from upstream 1.18.9)", () => {
  test("preserves content when structuredContent is also present", async () => {
    ...
    const structuredContent = { type: "nuwax.openui-ref", artifactId: "x" }
  ...
  test("falls back to structuredContent text only when content is absent", async () => {
  ...
  test("throws when MCP result isError", async () => {
